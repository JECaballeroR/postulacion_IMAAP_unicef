...
def show_table(data):
    ...
    fig = go.Figure(
        data=[
            go.Table(
                header=dict(
                    values=list(df.columns), fill_color="#262730", align="center"
                ),
                cells=dict(
...
def cargar_datos():
    ...
    dem_vacante = pd.read_excel(
        "ExcelConsolidado.xlsx", sheet_name="Dem. Laboral por tipo vacante"
    )

    data["Depto"] = data["Depto"].apply(
        lambda x: x.replace("BOGOTA, D. C.", "CUNDINAMARCA")
    )
...
meses = st.sidebar.multiselect(
    "Mes",
    options=dem_vacante["Mes"].unique(),
    default=dem_vacante["Mes"].unique(),
)
...
sector = st.sidebar.multiselect(
    "Sector",
    options=dem_vacante["nombre_sector"].unique(),
    default=dem_vacante["nombre_sector"].unique(),
)

df_sector = filtar_df(df_meses, sector, "nombre_sector")


df_sector = df_sector.melt(
    id_vars=["nombre_sector", "ciuo 4d", "Año", "Mes", "ciiu2dig", "ciuo_3d"],
    value_vars=["ARS", "NS", "ARDC", "DCS", "DCN"],
    var_name="Tipo Vacante",
    value_name="Vacantes",
)
if sector:
    tipo = st.sidebar.multiselect(
        "Tipo Vacante",
        options=df_sector["Tipo Vacante"].unique(),
        default=df_sector["Tipo Vacante"].unique(),
    )

    df_tipo = filtar_df(df_sector, tipo, "Tipo Vacante")
    df_mostrar = (
        df_tipo.groupby("Tipo Vacante")
        .sum()
        .reset_index()[["Tipo Vacante", "Vacantes"]]
    )
    ...
    data_table = pd.DataFrame(
        df_mostrar[
            df_mostrar["Tipo Vacante"].apply(lambda x: x in ["ARS", "ARDC", "DCS"])
        ]
    )
    ...
    col2.plotly_chart(
        plot_heatmap(
            heatmap_df, x="Tipo Vacante", y="Actividad (2 digitos)", val="Vacantes"
        ),
        use_container_width=True,
    )
    df_bar = (
        df_tipo.groupby("nombre_sector")
        .sum()
        .reset_index()[["nombre_sector", "Vacantes"]]
        .sort_values(by="Vacantes", ascending=False)
    )
    col2.markdown("""### Vacantes por Sector""")

    col2.plotly_chart(
        px.bar(df_bar, x="nombre_sector", y="Vacantes"), use_container_width=True
    )
    col1.plotly_chart(
        px.bar(data_table.sort_values(by="Vacantes"), x="Tipo Vacante", y="Vacantes"),
        use_container_width=True,
    )
else:
