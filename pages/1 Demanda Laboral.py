...
st.markdown(
    """Con la información anterior realizar un Dashboard (obviando las vacantes sin
...
meses = st.sidebar.multiselect(
    "Mes",
    options=data["Mes"].unique(),
    default=data["Mes"].unique(),
)
...
dpto = st.sidebar.multiselect(
    "Departamento",
    options=df_meses["Depto"].unique(),
    default=df_meses["Depto"].unique(),
)
...
def plot_ciiu2dig(df):
    ...
    data_vacantes = (
        data_vacantes.groupby("ciiu2dig")
        .sum()
        .reset_index()
...
if dpto:
    mpio = st.sidebar.multiselect(
        "Municipio", options=df_dpto["Mpio"].unique(), default=df_dpto["Mpio"].unique()
    )

    df_mpio = filtar_municipio(df_dpto, mpio=mpio)

    st.markdown(
        f'# TOTAL DE VACANTES DE LA SELECCIÓN: <span style="color:#4BAAFF"> {df_mpio["Vacantes"].sum()} </span>',
    ...
    col1.dataframe(
        (
            df_mpio[["Mpio", "ciiu2dig", "ciuo_3d", "Vacantes"]]
            .groupby(["Mpio", "ciiu2dig", "ciuo_3d"])
            .sum()
            .reset_index()
            .sort_values(by="Vacantes", ascending=False)
        )
