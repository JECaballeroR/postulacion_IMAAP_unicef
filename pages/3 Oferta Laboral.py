...
def cargar_datos():
    registrados = pd.read_excel(
        "ExcelConsolidado.xlsx", sheet_name="Oferta Laboral - Registrados"
    ).dropna()
    colocados = pd.read_excel(
        "ExcelConsolidado.xlsx", sheet_name="Oferta Laboral - Colocados"
    )

    colocados["Departamento Residencia "] = colocados["Departamento Residencia "].apply(
        lambda x: x.replace("BOGOTÁ, D.C.", "CUNDINAMARCA")
    )
    registrados["Departamento_Residencia "] = registrados[
        "Departamento_Residencia "
    ].apply(
        lambda x: str(x).replace("BOGOTÁ, D.C.", "CUNDINAMARCA")
        if isinstance(x, str)
        else x
    )
    colocados = colocados[
        colocados["ciiu2dig"].apply(lambda x: x not in ["NA", "nan", "na"])
    ]
    with urlopen(
...
mes = st.sidebar.multiselect(
    "Meses", options=registrados["Mes"].unique(), default=registrados["Mes"].unique()
)
...
registrados_agrupado = (
    registrados.groupby(["Año", "Mes", "País", "Departamento_Residencia "])
    .sum()
    .reset_index()
)
...
col1.plotly_chart(
    plot_map(
        registrados_agrupado,
        col_departamento="Departamento_Residencia ",
        col_data="Registrados",
    )
)

colocados_agrupado = (
    colocados.groupby(["Año", "Mes", "País", "Departamento Residencia "])["Colocados"]
    .sum()
    .reset_index()
)
...
col2.plotly_chart(
    plot_map(
        colocados_agrupado,
        col_departamento="Departamento Residencia ",
        col_data="Colocados",
    )
)
